...
LSM9DS0::LSM9DS0()
{
    ...
    Serial.println("Checking for LSM9DS0");
	...
	{
	  Sensor.begin();
      Serial.println("LSM9DS0 available");
...
void LSM9DS0::HandleTime(unsigned int ElapsedTime)
{
...
bool LSM9DS0::isShaked ()       // True if the sensor is shaked
{
    if(Shaked)
    {
      Shaked = false;
...
bool LSM9DS0::isTapped ()       // True if the sensor has been tapped once
{
    if(Tapped)
    {
      Tapped = false;
...
bool LSM9DS0::isDoubleTapped () // Returns true if the sensor has been double tapped
{
    if(DoubleTapped)
    {
      DoubleTapped = false;
...
int LSM9DS0::isTilted ()        // Return the tilt axis according to enum Axis, else returns false
{
    if(Tilted)
    {
        Tilted = false;
...
bool LSM9DS0::isActive()        // Return the state of the sensor (true = active, false = inactive)
{
    return State;
}
...
void LSM9DS0::RefreshValues() // This function has to be adapted to the current used sensor
{
 ...
 *********************************************/
    ...
    if(ShakeTilt_Timing >= SHAKE_TILT_TIME)
    {
        ...
        {
            Tilted = true;
            TiltAxis = LSM9DS0::Y_AXIS | LSM9DS0::X_AXIS;
        }
        
        else if(abs(a_x_Filtered) > ACC_TILT_THRESH)
        {
            Tilted = true;
            TiltAxis = LSM9DS0::X_AXIS;
        }
        
        else if(abs(a_y_Filtered) > ACC_TILT_THRESH)
        {
            Tilted = true;
            TiltAxis = LSM9DS0::Y_AXIS;
        }
